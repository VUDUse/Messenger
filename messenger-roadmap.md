...
Разбит на фазы. Внутри каждой фазы — отдельные коммиты, рассчитанные на цикл "раз в 1-2 дня".
Правила для коммитов:
- Один коммит = одно завершённое небольшое изменение (не смешивать рефактор и фичу).
- Коммит-сообщения в формате `feat: ...`, `fix: ...`, `refactor: ...`, `chore: ...`.
- Заводи Issue в GitHub на каждый пункт ниже и закрывай его коммитом/PR — это тоже считается в активность профиля.
...
**Важно:** если старая БД уже содержит пароли в открытом виде — на этом шаге нужен разовый скрипт-миграция для перехеширования существующих записей.
...
**Важно:** пока `EmailConfirmed = false`, пользователю нельзя логиниться — учётку можно создавать сразу, но входить разрешать только после подтверждения кода (либо не создавать запись в `Users` до подтверждения, а хранить "pending regisration" отдельно — надёжнее, чтобы не плодить неподтверждённые аккаунты).
...
- [ ] `feat`: запись аудио через NAudio, отправка как voice-message (waveform/длительность желательно, но не обязательно на первом шаге)
...
- [ ] `refactor`: полностью убрать `CheckIPAddressLogin` (нелогичный и небезопасный механизм)
- [ ] `refactor`: `AuthService` — хранить токен не в реестре, а в файле `%AppData%\Messenger\auth.dat`, шифруя через `System.Security.Cryptography.ProtectedData` (DPAPI, не требует прав администратора и не завязан на реестр)
...
- [ ] `chore`: GitHub Actions workflow — сборка проекта на каждый push/PR (зелёный бейджик в репозитории, плюс это реально полезно — ловит поломки сборки)
...
При выбранном темпе (раз в 1-2 дня) фазы 0-5 — это ориентировочно 8-12 недель при 1 коммите за сессию.
Рекомендуемый порядок: **0 → 1 → 2 → 3 → 5 → 4 → 6 → 7** (регистрация/почта сразу после безопасности, так как модель `Users` — фундамент для всего остального; Realtime после файлов и автологина, потому что Фаза 4 — самая объёмная по архитектуре, лучше подойти к ней на "разогретом" проекте).
